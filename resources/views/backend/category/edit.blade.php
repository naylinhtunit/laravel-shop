    ...
    <div class="card-body">
      ...
      <div class="form-group {{(($category->is_parent==1) ? 'd-none' : '')}}" id='parent_cat_div'>
        ...
        <div class="form-group">
          ...
          <input type="checkbox" name='is_child_parent' id='is_child_parent' value='{{$category->is_child_parent}}' {{(($category->is_child_parent==2)? 'checked' : '')}}> Yes                        
        </div>

      <div class="form-group {{(($category->is_child_parent==2) ? 'd-none' : '')}}" id='child_parent_cat_div'>
          <label for="parent_id">Child Parent Category</label>
          <select name="parent_id" class="form-control">
              <option value="">--Select any category--</option>
              @foreach($child_parent_cats as $key=>$child_parent_cat)
              
                  <option value='{{$child_parent_cat->id}}' {{(($child_parent_cat->id==$category->parent_id) ? 'selected' : '')}}>{{$child_parent_cat->title}}</option>
              @endforeach
          </select>
        </div>
